...
**新功能**
- 新增保存 MD 文件按钮，位于编辑器工具栏，一键下载当前内容
- 导出文件名自动取正文第一个标题，无标题时使用默认名

**修复**
- 修复图片导出有序列表序号显示为上标的问题（html2canvas ::marker 兼容）
- 修复图片导出嵌套列表序号渲染异常，支持任意层级嵌套
- 修复图片生成 DOM 清理 race condition，改用 finally 块统一处理
- 修复导出文件名含特殊字符（\/:*?"<>|）导致下载异常的问题
- 修复 localStorage 加载图片设置时 JSON 解析异常可能导致崩溃的问题

**优化**
- 图片导出补全引用块、行内代码、表格单元格、分隔线、链接的显式样式
- 修正图片导出行高（1.8→1.6）与预览区保持一致
- 修正图片导出表格 borderCollapse 与 borderSpacing 冲突
- 编辑器输入事件中 saveContent 和 updateWordCount 添加 debounce，减少大文档卡顿
